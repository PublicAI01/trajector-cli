...
func TestEnableRollbackKeepsGitIgnoreLinesAddedMeanwhile(t *testing.T) {
	...
	err := e.machine().Enable(e.project, choices(proxytest.WithProxy), lifecycle.IO{In: strings.NewReader(e.stdin), Out: out, Err: e.stderr})
	if err == nil {
		t.Fatal("enable succeeded while capture was paused")
	}
...
func TestDisableRemovesAnInjectionWithoutBaseURLAndWithoutRoute(t *testing.T) {
	...
	if st := e.status(); !st.Injected || st.InjectedBaseURL != "" {
