...
// installation. A managed installation is upgraded through its manager
// rather than by overwriting the binary: the manager tracks a version
// and a receipt, and a file it did not put there is either reverted by
// the next command or left as an inconsistency the user has to unpick.
const (
	Homebrew = "homebrew"
	Scoop    = "scoop"
)
...
// from.
func managerForPath(path string) string {
